...
function CartItem({ checkedItemArrayPush, setQuantity, id, check, quantity, name, img_url, price_origin, sale, wrap, present }) {
  ...
  useEffect(() => {
    setLastPrice(reNumber(String(itemQuantity * price_origin * (1 - sale))));
    setTotalPrice(reNumber(String(itemQuantity * price_origin)));
  }, [itemQuantity]);
